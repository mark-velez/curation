...
def dataset_properties_from_file(json_path: str) -> dict:
    """
    Read and validate a JSON file containing dataset access properties
    (Structure described at https://tinyurl.com/269xprpe)

    :param json_path: path to a dataset resource file that defines dataset access
    :return: a dict mapping API field =>
    """
    with open(json_path, 'r', encoding='utf-8') as json_fp:
        props = json.load(json_fp)
    if 'access' not in props:
        raise RuntimeError(f'Missing "access" field in {json_path}')
    if not isinstance(props['access'], list):
...
def load(project_id: str,
    ...
    :param access_entries: dataset access controls to assign to the destination dataset
...
def main(project_id: str,
    ...
    :param dataset_json_path: path to a JSON file that defines dataset access 
      (structure described at https://tinyurl.com/269xprpe)
    """
    props = dataset_properties_from_file(dataset_json_path)
    access_entries = [
        bigquery.AccessEntry.from_api_repr(access_entry)
        for access_entry in props['access']
    ]
    ...
    dst_dataset = load(project_id, bq_client, staging_dataset.dataset_id, dst_dataset_id)

    # to prevent sharing dataset that fails to load
    # only set dataset access policy at the end
    dst_dataset.access_entries = access_entries
