...
def run_achilles_helper(hpo_id, folder_prefix, bucket):
    ...
    try:
        ...
        if err.resp.status == 400:
            reason = err.content
            if err.resp.get(main_constants.CONTENT_TYPE, '').startswith(main_constants.APPLICATION_JSON):
                err_content = json.loads(err.content)
                reason = err_content.get(main_constants.ERROR).get(main_constants.ERRORS)[0].get(main_constants.REASON)
...
def process_hpo(hpo_id, force_run=False):
    ...
    if folder_prefix is None:
        ...
        if all_required_files_loaded(results):
            ...
            heel_error_rows = main_constants.HEEL_ERROR_FAIL_ROWS
...
def get_heel_error_query(hpo_id):
    ...
    :return: the query
    """
    table_id = bq_utils.get_table_id(hpo_id, main_constants.ACHILLES_HEEL_RESULTS_TABLE)
    return render_query(main_constants.HEEL_ERROR_QUERY_VALIDATION,
                        table_id=table_id)
...
def get_duplicate_counts_query(hpo_id):
    ...
    for table_name in cdm.tables_to_map():
        ...
        if table_id in all_table_ids:
            sub_query = render_query(main_constants.DUPLICATE_IDS_SUBQUERY, table_name=table_name, table_id=table_id)
            sub_queries.append(sub_query)
    unioned_query = main_constants.UNION_ALL.join(sub_queries)
    return main_constants.DUPLICATE_IDS_WRAPPER.format(union_of_subqueries=unioned_query)
...
def get_drug_class_counts_query(hpo_id):
    ...
    :return: the query
    """
    table_id = bq_utils.get_table_id(hpo_id, main_constants.DRUG_CHECK_TABLE)
    return render_query(main_constants.DRUG_CHECKS_QUERY_VALIDATION, table_id=table_id)
